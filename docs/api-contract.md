...
Удаляет один серверный чат вместе с его сообщениями, если в будущем будет реализовано серверное хранение истории. В MVP удаление отдельного чата выполняется локально на frontend.

### `DELETE /api/chats`

Очищает всю серверную историю пользователя, если в будущем будет реализовано серверное хранение истории. В MVP очистка всей истории выполняется локально на frontend.
